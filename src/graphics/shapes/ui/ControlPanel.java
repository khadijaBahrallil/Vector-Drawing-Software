...
import graphics.shapes.save.Export;
...
public class ControlPanel extends JMenuBar {
	...
	public ControlPanel(ShapesView shapesView) throws FileNotFoundException {
		...
		menuExport = new JMenuItem(" Export as JPG ");
		menuExport.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {

				export.takePicture(shapesView);
		...
		mStar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				Point[] points = { new Point(27, 198), new Point(153, 162), new Point(153, 18), new Point(234, 135),
						new Point(360, 90), new Point(288, 198), new Point(360, 306), new Point(234, 270),
						new Point(153, 387), new Point(153, 243) };
		...
		JMenuItem mPathTest = new JMenuItem("Path Test");
 		mPathTest.addActionListener(new ActionListener() {
 			@Override
 			public void actionPerformed(ActionEvent arg0) {
 				System.out.println("Création Path test");

 				SPath pathTest = new SPath();
 				pathTest.addAttributes(new ColorAttributes(false,true,Color.YELLOW,Color.BLACK));
 				pathTest.addAttributes(new SelectionAttributes());
 				pathTest.addAttributes(new PathAttributes(Type.method_Test,"IsDown"));
 				model.add(pathTest);
 				ShapesController.getView().repaint();
 			}	
 		});
 		menuShape.add(mPathTest);
